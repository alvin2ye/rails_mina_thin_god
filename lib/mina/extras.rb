...
task :create_extra_paths do
  ...
  queue 'echo "-----> Copy Config File"'
  queue echo_cmd "echo vi #{deploy_to}/#{shared_path}/config/database.yml"
  queue echo_cmd "echo vi #{deploy_to}/#{shared_path}/config/config.yml"
  queue echo_cmd "echo vi #{deploy_to}/#{shared_path}/config/initializers/secret_token.rb"  
end
